...
type Config struct {
	...
	ServerTimeoutSecs int    `mapstructure:"SERVER_TIMEOUT_SECONDS"` // Read as int
	ServerTimeout     time.Duration // Actual duration, populated after load
	...
	DBConnMaxLifetimeMins int    `mapstructure:"DB_CONN_MAX_LIFETIME_MINUTES"` // Read as int
	DBConnMaxLifetime     time.Duration // Actual duration, populated after load
...
func Load() (*Config, error) {
	...
	v.SetDefault("SERVER_TIMEOUT_SECONDS", 30) // Default as int
	...
	v.SetDefault("DB_CONN_MAX_LIFETIME_MINUTES", 60) // Default as int
	...
	// Populate actual duration fields from the integer values read by Viper
	cfg.ServerTimeout = time.Duration(cfg.ServerTimeoutSecs) * time.Second
	cfg.DBConnMaxLifetime = time.Duration(cfg.DBConnMaxLifetimeMins) * time.Minute
